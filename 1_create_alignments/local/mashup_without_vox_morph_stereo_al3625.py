...
musdb_data = pd.read_csv("/home/lankaraniamir/kaldi-trunk/egs/lyric_separator/db/musdb_split/musdb_all_data.csv")

MUSDB_PATH = "/home/lankaraniamir/kaldi-trunk/egs/lyric_separator/db/musdb18hq"
# EXTRA_WAV_PATH = "/home/lankaraniamir/kaldi-trunk/egs/lyric_separator/db/musdb18"
EXTRA_WAV_PATH = "/home/lankaraniamir/kaldi-trunk/egs/lyric_separator/db/my_stems"
OUTPATH = Path.home() / 'kaldi-trunk/egs/lyric_separator/db/mashups_no_morph/'
CACHE = Path.home() / 'kaldi-trunk/egs/lyric_separator/demucs/automix_cache/'

# MUSDB_PATH = '/checkpoint/defossez/datasets/musdbhq'
# MUSDB_PATH = "/home/lankaraniamir/kaldi-trunk/egs/lyric_separator/db/musdb18"
...
def analyse_track(dset, index):
    ...
    # lyric_times.append(relevant_lines.stop.values[-1])
...
def build_track(ref_index, catalog):
    ...
    """
    # Edited
    # Index 3 = vocals if not randomized so
    # avoid randomization so we can isolate Vox
    ...
    # final_times = [time for time in new_times if time<audio_length]
    # final_lyrics = [new_lyrics[i] for i, time in enumerate(new_times) if time<audio_length]
    return stems, origs, final_lyrics, final_times, audio_length

    # stems, final_lyrics, final_times, audio_length = align_stems(stems, onset_lyrics, onset_times)
    # stems, final_lyrics, final_times, audio_length = align_stems(stems,
    # onset_lyrics, onset_times)

    # print("FINAL CHOICES", ref_index, indexes, dps, dts)
    # print("AFTER: ", spec_ref.onsets[:10], spec_ref.lyric_times,onset_lyrics[:10],"\n")
    # print(spec_ref.onsets[:10], spec_ref.lyric_times,onset_lyrics[:10])
    # print("***", spec_ref.onsets)
    # for src in order[1:]:
    # print(" \n\n")
    # print(stems[1], "\n")
    # print(stems[1])
...
def main():
    ...
    for copy in range(copies):
        ...
        for index in range(len(catalog)):
            ...
            # catalog[index].lyrics
            # catalog[index].lyric_times
            # catalog[index].audio_length
            mix = track.sum(0)
            mx = mix.abs().max()
            scale = max(1, 1.01 * mx)
            mix = mix / scale
...
# shifted_times = [abs(onset + v_shift/1000) for onset in best_times]
# start_index = shifted_times.index(min(shifted_times))

# print("\nFINAL ONSETS : ", [round(o, 3) for o in best_onsets])
# shifted_times = [abs(onset + v_shift*width) for onset in best_times]
# shifted_times = [abs(onset + v_shift/SR) for onset in best_times]

# print("\nSHIFTED ONSETS: ", [round(o, 3) for o in shifted_times])

# Round to .5
# final_times = [round(time, 2) for time in shifted_times[start_index:]]
# final_times[0] = max(final_times[0], 0)
# final_lyrics = best_lyrics[start_index:]

# print("\nFINAL TIMES: ", final_times)
# print("valid_times: ", valid_times)
# final_times = [*set(valid_times)]
# print("final_times: ", final_times)
# print("final_times (after): ", final_times)
# final_lyrics = [*set(onset_lyrics[start_index:])]
# print("final_lyrics: ", final_lyrics)
...
# print(shifts[:10])
# print(shifts[-10:])
# print([shift/max_shift for shift in shifts])
# print([(shift-new_zero)/max_shift for shift in shifts])
# quit()

# (v_wav, v_onsets) = stems[3]
# v_shift = shifts[3]
# offset = v_shift - new_zero
# v_wav = F.pad(v_wav, (offset, 0))
# outs.append(v_wav)
...
# print("\nv_onsets")
# print(v_onsets)
# print(v_onsets*width)
# quit()
# onset_index = np.where(v_onsets == v_shift)[0]
# print(onset_index)
# print(v_shift, onset_index, onset_times[onset_index], onset_lyrics[onset_index])
# quit()
# offset = shift - new_zero
# wav = F.pad(wav, (offset, 0))
# outs.append(wav)
# for (wav, onsets), shift in zip(stems[3], shifts[3]):
#   onset_pos = onsets.index(shift)
#   print(shift, onset_pos, onset_lyrics[onset_pos], lyrics[onset_pos])
#   quit()
#   offset = shift - new_zero
#   wav = F.pad(wav, (offset, 0))
#   outs.append(wav)
